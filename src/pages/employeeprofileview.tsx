...
const EmployeeProfile: React.FC = () => {
  ...
  // Validate citizenship input (only letters and spaces)
  const validateCitizenship = (value: string): boolean => {
    if (value === '') return true;
  ...
  // Store original Date of Birth field ID to maintain citizenship field
  const [dobFieldId, setDobFieldId] = useState<string>('dob');

  // Update DOB field ID when personal fields change
  useEffect(() => {
    const dobField = personalFields.find(field => field.id === 'dob');
    if (dobField) {
      setDobFieldId(dobField.id);
    }
  }, [personalFields]);
  ...
  // Improved input handlers that preserve cursor position
  const handleLabelInput = (e: React.FormEvent<HTMLSpanElement>, section: string, field: Field) => {
    ...
    if (validateLabel(value)) {
      ...
      setTimeout(() => restoreCursorPosition(element, cursorPos), 0);
    }
  };

  const handleValueInput = (e: React.FormEvent<HTMLDivElement>, section: string, field: Field) => {
    const element = e.currentTarget;
    const cursorPos = saveCursorPosition(element);
    const value = element.textContent || '';
  ...
  // Handle citizenship input with cursor preservation
  const handleCitizenshipInput = (e: React.FormEvent<HTMLDivElement>, citizenshipField: CitizenshipField) => {
    const element = e.currentTarget;
    const cursorPos = saveCursorPosition(element);
    const value = element.textContent || '';
    
    if (validateCitizenship(value)) {
      updateCitizenshipField(citizenshipField.id, value);
      // Restore cursor position after state update
      setTimeout(() => restoreCursorPosition(element, cursorPos), 0);
    }
  };

  // Handle key press for better validation
  const handleKeyPress = (e: React.KeyboardEvent, validationFn: (value: string) => boolean, currentLabel?: string) => {
    const char = e.key;
    ...
    // Get current text and simulate the new text after this key press
    ...
    if (selection && selection.rangeCount > 0) {
      ...
      newText = currentText.substring(0, start) + char + currentText.substring(end);
    } else {
      newText = currentText + char;
    }
    
    if (!validationFn(newText)) {
      e.preventDefault();
    }
  };

  // Dynamic field component with improved editing
  ...
  <div className={styles.fieldFormat}>
    <div className={section === 'personal' ? styles.personalInfoFormat : styles.mainInfoFormat}>
      ...
      <span 
        ...
        onInput={() => {}} // Prevent cursor disruption
        onKeyPress={(e) => handleKeyPress(e, validateLabel)}
        onBlur={(e) => {
          const value = e.currentTarget.textContent?.trim() || field.label;
          if (validateLabel(value)) {
            updateFieldLabel(section, field.id, value);
          } else {
      ...
      <div 
        ...
        suppressContentEditableWarning={true}
        onInput={() => {}} // Prevent cursor disruption
        onKeyPress={(e) => handleKeyPress(e, (val) => validateInput(val, field.label), field.label)}
        onBlur={(e) => {
          const value = e.currentTarget.textContent?.trim() || field.value;
          if (validateInput(value, field.label)) {
            updateFieldValue(section, field.id, value);
          } else {
    ...
    <div className={styles.employeeBoxFormat}>
      ...
      <div className={styles.backgroundFrameBox}>
        ...
        <div className={styles.informationBody}>
          ...
          <div className={styles.personalInformationBox} style={{ overflow: 'hidden' }}>
            {/* Personal Header */}
                ...
                {field.id === dobFieldId && (
                  <div className={styles.fieldFormat} id="citizenship-field">
                    <div className={styles.personalInfoFormat}>
                      ...
                      <div 
                        ...
                        onInput={(e) => {
                          ...
                          if (validateCitizenship(value)) {
                            // Don't need to save this to state as it's just the default field
                            setTimeout(() => restoreCursorPosition(element, cursorPos), 0);
                          }
                        }}
                        onKeyPress={(e) => handleKeyPress(e, validateCitizenship)}
                        onBlur={(e) => {
                          const value = e.currentTarget.textContent?.trim() || '';
                          if (!validateCitizenship(value)) {
                            e.currentTarget.textContent = 'Not Set';
                          }
                        }}
                      >
                        Not Set
                    ...
                    {citizenshipFields.length > 0 && (
                      <div className={styles.infoTextboxContainer}>
                        {citizenshipFields.map((citizenshipField) => (
                          <div
                            ...
                            onInput={(e) => handleCitizenshipInput(e, citizenshipField)}
                            onKeyPress={(e) => handleKeyPress(e, validateCitizenship)}
                            onBlur={(e) => {
                              const value = e.currentTarget.textContent?.trim() || citizenshipField.value;
                              if (validateCitizenship(value)) {
                                updateCitizenshipField(citizenshipField.id, value);
                                // If empty or "not set", remove the field
                                if (!value || value.toLowerCase() === 'not set') {
                                  removeCitizenshipField(citizenshipField.id);
                                }
                              } else {
                                e.currentTarget.textContent = citizenshipField.value;
                              }
                            }}
                          >
          ...
          <div className={styles.mainInformationBox} style={{ overflow: 'hidden' }}>
            {/* Main Header */}
